...
<?php
if($_POST['id_1_edit'] != "00"){
	//MODIFICA
	$titolo = $_POST['titolo_1'];
	$url = $_POST['url_1'];
	if($url == null || $url == ""){
		$url = "";
	}else{
	...
	$query = 'UPDATE pubblicita SET titolo = "' . $arg1  .'", url = "' . $arg2  .'" WHERE id = '. $_POST['id_1_edit'];
	...
	//campi
	$titolo = $_POST['titolo_1'];
	$immagine_tmp_name = $_FILES['immagine_1']['tmp_name'];
	$immagine_size = $_FILES['immagine_1']['size'];
	$url = $_POST['url_1'];
	...
	if($immagine_tmp_name == null){
		...
		$arg1 = mysql_real_escape_string($titolo);
		$arg2 = mysql_real_escape_string($url);
		$query = 'INSERT INTO pubblicita (titolo, immagine, url) VALUES ("'.$arg1.'", null, "'.$arg2.'")';
	...
	if($flag != "" ){
		...
		if($flag == "img"){
			...
			$arg1 = mysql_real_escape_string($titolo);
			$arg2 = mysql_real_escape_string($url);
			$query = 'INSERT INTO pubblicita (titolo, immagine, url) VALUES ("'.$arg1.'", "temp", "'.$arg2.'")';
			$result = mysql_query($query, $db) or die(mysql_error($db));
			...
			$query = 'UPDATE pubblicita SET immagine = "' . $imagename . '" WHERE immagine = "temp" and id = '.$last_id;
			$result = mysql_query($query, $db) or die(mysql_error($db));
			...
			switch($type){
				case IMAGETYPE_PNG:
					imagepng($new_image, $fld_pubblicita . '/' . $imagename);
					imagepng($thumb, $fld_pubblicita_thumbs . '/' . $imagename);
					break;
			}
			imagedestroy($image);
